...
| 指标 | 数值 |
|------|------|
| 在拍条目 | **544 条**（原始命中 4,494 条，经口径过滤 + 去重 + 详情页二次过滤） |
| 覆盖拍行 | **60+ 家**（Adam's、Galerie Zacke、Hannam's、Aalders、Antonia、Fung Ngai HK、Golden State 等） |
| 带图 | 540 / 544 |
| 带估价 | 204 条 |
| 年代分布 | 高古 170 · 清 100 · 元明 9 · 唐宋辽金 6 · 未标注 259 |
| 有产出的源 | 6 / 76：Auctionet、HiBid、Invaluable、the-saleroom、i-bidder、Sotheby's |

**为什么只有 6 个源有产出，却能覆盖 60+ 家拍行**：这 6 个里有 4 个是聚合平台
（HiBid 聚合北美数千家、the-saleroom 聚合英国上千家、i-bidder 同属 ATG、
Invaluable 全球聚合），一个源就带出几十家中小拍行的拍品。

**其余 70 个源为什么没产出**（页面「抓取源状态」里逐个如实标注）：

| 状态 | 家数 | 说明 |
|------|------|------|
| 检索地址无效 404 | ~30 | 我推测的搜索 URL 与该行实际路径不符，需按探测的链接形态逐个校正 |
| 反爬拦截 403 | ~8 | Bonhams、Heritage、Catawiki、Michaan's 等在 CDN 层直接拒绝数据中心 IP |
| 人机挑战页 202 | ~8 | ATG 系与部分英国行返回 Cloudflare 挑战 |
| 页面可取但未解析出拍品 | ~24 | 搜索页确实打开了，但结果区结构尚未适配 |

这是逐步收敛的工程：每轮探测（`[probe]` 提交或手动 dispatch）会打印各家的
「站内链接形态直方图」，据此校正 `sources.yml` 里的搜索地址与 lot 链接规律。
